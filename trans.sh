...
if defined ipv4_addr if defined ipv4_gateway if defined ipv4_mask (
    ...
    netsh interface ipv4 set address %id% static !ipv4_addr! !ipv4_mask! !ipv4_gateway! 1 >nul
    call :apply_ipv4_dns
) else (
    echo Enforcing DHCP IPv4
    netsh interface ipv4 set address %id% dhcp >nul
    netsh interface ipv4 set dnsservers %id% dhcp >nul
)
...
if defined ipv6_addr if defined ipv6_gateway if defined ipv6_prefix (
    ...
    netsh interface ipv6 add address %id% !ipv6_addr!/!ipv6_prefix! >nul
    netsh interface ipv6 add route ::/0 %id% !ipv6_gateway! >nul
    call :apply_ipv6_dns
) else (
...
:resolve_interface
set "id="
if defined mac_addr call :find_interface_by_mac
...
for /f %%a in ('powershell -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass ^
    -Command "$m='%mac_addr%';(Get-CimInstance Win32_NetworkAdapter -ErrorAction SilentlyContinue | Where-Object { $_.MACAddress -eq $m } | Sort-Object InterfaceIndex | Select-Object -First 1 -ExpandProperty InterfaceIndex)" ^| findstr "^[0-9][0-9]*$"') do set "id=%%a"
goto :eof

:find_interface_auto
for /f %%a in ('powershell -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass ^
    -Command "(Get-NetAdapter -ErrorAction SilentlyContinue | Where-Object { $_.HardwareInterface -eq $true -and $_.Status -ne ''Disabled'' -and $_.Name -notmatch ''Loopback|Pseudo|Teredo|isatap'' } | Sort-Object ifIndex | Select-Object -First 1 -ExpandProperty ifIndex)" ^| findstr "^[0-9][0-9]*$"') do set "id=%%a"
if defined id goto :eof
for /f %%a in ('powershell -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass ^
    -Command "(Get-WmiObject Win32_NetworkAdapter -ErrorAction SilentlyContinue | Where-Object { $_.NetEnabled -eq $true -and $_.MACAddress -ne $null } | Sort-Object InterfaceIndex | Select-Object -First 1 -ExpandProperty InterfaceIndex)" ^| findstr "^[0-9][0-9]*$"') do set "id=%%a"
goto :eof

:apply_ipv4_dns
if not defined ipv4_dns1 goto :eof
echo Applying IPv4 DNS override
netsh interface ipv4 set dnsservers %id% static %ipv4_dns1% primary >nul
set /a i=2
:loop_ipv4_dns
call set "cur=%%ipv4_dns%i%%%"
if not defined cur goto :eof
netsh interface ipv4 add dnsservers %id% !cur! index=!i! >nul
set /a i+=1
goto loop_ipv4_dns
...
echo Applying IPv6 DNS override
netsh interface ipv6 set dnsservers %id% static %ipv6_dns1% primary >nul
set /a i=2
:loop_ipv6_dns
call set "cur=%%ipv6_dns%i%%%"
if not defined cur goto :eof
netsh interface ipv6 add dnsservers %id% !cur! index=!i! >nul
set /a i+=1
goto loop_ipv6_dns
